    ...
    for dialog in data:
        ...
        for tn in range(0, len(dialog['turns']['utterance']), 2):
            ...
            for sl, val in curr_state_dict.items():
                ...
                curr_state[domain][name] = val
            turn = create_turn(context, context_size, dialogue_id, tn, dialog, last_state, domain_gt, state_update, curr_state ,database_results)
            yield turn
...
def create_turn(context, context_size, dialogue_id, tn, dialog, last_state, domain_gt, state_update,parsed_state ,database_results):
    return {
